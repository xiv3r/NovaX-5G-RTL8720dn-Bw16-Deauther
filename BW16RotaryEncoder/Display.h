...
void drawBorders(float progress) {
    // 화면의 중앙 X 좌표 (128x64 디스플레이 기준)
    const int screenCenterX = 64;

    // 상단 왼쪽 선: 중앙에서 왼쪽으로 확장 (최종 X: 25)
    int leftLineEndX1 = screenCenterX - (int)((screenCenterX - 25) * progress);
    display.drawLine(screenCenterX, 0, leftLineEndX1, 0, WHITE);

    // 상단 오른쪽 선: 중앙에서 오른쪽으로 확장 (최종 X: 103)
    int rightLineEndX1 = screenCenterX + (int)((103 - screenCenterX) * progress);
    display.drawLine(screenCenterX, 0, rightLineEndX1, 0, WHITE);

    // 하단 왼쪽 선: 중앙에서 왼쪽으로 확장 (최종 X: 25)
    int leftLineEndX2 = screenCenterX - (int)((screenCenterX - 25) * progress);
    display.drawLine(screenCenterX, 63, leftLineEndX2, 63, WHITE);

    // 하단 오른쪽 선: 중앙에서 오른쪽으로 확장 (최종 X: 103)
    int rightLineEndX2 = screenCenterX + (int)((103 - screenCenterX) * progress);
    display.drawLine(screenCenterX, 63, rightLineEndX2, 63, WHITE);
}
void draw_menu(std::vector<String>& menu_name, int selected) {
    ...
    const unsigned long lineAnimDuration = 250UL;
    const unsigned long lineDelay = 80UL;
    ...
    if (prev_selected == -1 || menu_changed) {
        ...
        while (millis() < endTime) {
            ...
            float progress = float(now - startTime) / float(initialAnimTotalDuration);
            if (progress > 1.0f) progress = 1.0f;

            display.clearDisplay();
            drawBorders(progress);  // 애니메이션 진행도에 따라 테두리 그리기
            ...
            for (int i = 0; i < maxVisibleItems && (finalStartIndex + i) < menuSize; ++i) {
                ...
                int startInitialY = (display.height() > 0 ? display.height() : 64) + textOffsetY + (i * lineHeight);
            ...
            int startSelectedY = (display.height() > 0 ? display.height() : 64) + textOffsetY + ((selected - finalStartIndex) * lineHeight);
    ...
    if (prev_selected == -1 || prev_selected == selected) {
        ...
        display.clearDisplay();
        drawBorders(1.0f);  // 애니메이션 없이 최종 상태로 그리기
        drawScrollbar(selected, menuSize, maxVisibleItems, lineHeight, scrollBarWidth);
...
void Draw_Settings() {
    ...
    while (true) {
        ...
        if (ButtonPress(btnUp)) { // 값 또는 메뉴 인덱스 증가 (아래로)
            if (toggle_ok) {
        ...
        if (ButtonPress(btnDown)) { // 값 또는 메뉴 인덱스 감소 (위로)
            if (toggle_ok) {
                switch (selected_settings) {
                    case 0: if (frames_per_deauth > 1) frames_per_deauth--; break;
                    case 1: if (frames_per_becaon > 1) frames_per_becaon--; break;
                    case 2: if (max_clone > 1) max_clone--; break;
                    case 3: if (max_spam_space > 1) max_spam_space--; break;
                }
            } else {
...
void Draw_Selected_Menu() {
    ...
    while (true) {
        ...
        if (ButtonPress(btnUp)) {  // 위로 스크롤
            if (scrollindex < scan_results.size() - 1) {
                prev_scrollindex = scrollindex;  // 이전 인덱스 저장
                scrollindex++;                   // 현재 인덱스 증가
                prevPageStart = (prev_scrollindex / maxVis) * maxVis;  // 이전 페이지 시작점 계산
                pageStart = (scrollindex / maxVis) * maxVis;           // 현재 페이지 시작점 계산
                animate = (pageStart == prevPageStart);  // 수정된 부분: 같은 페이지일 때만 애니메이션
            }
        }

        if (ButtonPress(btnDown)) {  // 아래로 스크롤
            if (scrollindex > 0) {
                prev_scrollindex = scrollindex;  // 이전 인덱스 저장
                scrollindex--;                   // 현재 인덱스 감소
                prevPageStart = (prev_scrollindex / maxVis) * maxVis;  // 이전 페이지 시작점 계산
                pageStart = (scrollindex / maxVis) * maxVis;           // 현재 페이지 시작점 계산
                animate = (pageStart == prevPageStart);  // 수정된 부분: 같은 페이지일 때만 애니메이션
            }
